...
export class Projectile extends Sprite {
    ...
    update(deltaTime = 1000 / 60) {
        ...
        // radio de impacto basado en enemy (margen peque├▒o)
        const hitRadius = (this.enemy.radius ?? 6) + 2;

        // tama├▒o de substep en px: m├ís peque├▒o que el hitRadius para evitar saltos
        ...
        for (let s = 0; s < steps; s++) {
            ...
            // comprobar colisi├│n contra el objetivo actual
        ...
        // dibujar despu├⌐s de moverse (si no impact├│ y no est├í impactando)
        if (!collided && !this.impacting) {
            this.draw();
        }

        // si perdi├│ objetivo o sali├│ del canvas, lo limpias como antes
        if (!collided) {
