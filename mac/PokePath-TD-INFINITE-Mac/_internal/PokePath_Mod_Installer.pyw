...
if sys.platform == 'darwin':
    ...
    _app_bundle_env = os.environ.get('POKEPATH_APP_BUNDLE')
    APP_BUNDLE = Path(_app_bundle_env).expanduser().resolve() if _app_bundle_env else Path('/Applications/PokéPath TD.app')
    RESOURCES = APP_BUNDLE / "Contents" / "Resources"
    WORKSPACE = SCRIPT_DIR.parent
    WORKING_DIR = WORKSPACE / "working"
else:
    # Windows: mod lives inside game directory
    GAME_ROOT = SCRIPT_DIR.parent
    RESOURCES = GAME_ROOT / "resources"
    WORKING_DIR = RESOURCES  # extraction goes into resources/ on Windows

# Platform-appropriate font
FONT_FAMILY = 'Helvetica Neue' if sys.platform == 'darwin' else FONT_FAMILY
...
class FeatureSelectionDialog(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        # Bind mousewheel (Mac uses different delta values)
...
class ModInstaller(tk.Tk):
    ...
    def check_requirements_async(self):
        ...
        def check():
            ...
            has_game = RESOURCES.exists() and (RESOURCES / "app.asar").exists()
            npx_works, npx_error = check_npx_works()

            # Update UI from main thread
            self.after(0, lambda: self.on_requirements_checked(has_node, has_game, npx_works, npx_error))

        thread = threading.Thread(target=check, daemon=True)
        thread.start()

    def on_requirements_checked(self, has_node, has_game, npx_works, npx_error):
        """Handle requirements check result."""
        if not has_game:
            if sys.platform == 'darwin':
                self.set_status("⚠️ Game not found!\nInstall PokePath TD or set POKEPATH_APP_BUNDLE.", '#e94560')
            else:
    ...
    def on_install_error(self, error_msg):
        ...
        admin_tip = "• Try running as Administrator\n" if sys.platform == 'win32' else ""
        help_text = (
