...
try:
    ...
    try:
        ...
        import sys as _sys
        _flags = 0x08000000 if _sys.platform != 'darwin' else 0  # CREATE_NO_WINDOW (Windows only)
        _sp.check_call(['pip', 'install', 'Pillow', '-q'], creationflags=_flags)
        from PIL import Image, ImageTk
        HAS_PIL = True
    except Exception:
...
def find_paths():
    ...
    if sys.platform == 'darwin':
        # Mac: workspace has working/app_extracted with sprites
        check_dirs.append(script_dir.parent / 'working')
        # Also check the .app bundle for asar extraction
        app_bundle = Path(os.environ.get('POKEPATH_APP_BUNDLE', '/Applications/PokéPath TD.app'))
        if app_bundle.exists():
            check_dirs.append(app_bundle / 'Contents')
    else:
        check_dirs.extend([
    ...
    # On Mac, check workspace/working/app_extracted directly
    if sys.platform == 'darwin':
        workspace = script_dir.parent
        pokemon_base = workspace / 'working' / 'app_extracted' / 'src' / 'assets' / 'images' / 'pokemon'
...
def _is_game_modded():
    try:
        ...
        import sys
        if sys.platform == 'darwin':
            flag = Path.home() / 'Library' / 'Application Support' / 'pokePathTD_Electron_modded' / '.modded'
        else:
...
class App(tk.Tk):
        ...
        if self.save.load_from_game(modded=use_modded):
            ...
            if 'lock' in error_detail.lower() or 'running' in error_detail.lower():
                messagebox.showwarning("Load Failed",
                    ...
                    f"2. Check for 'node' processes and end them "
                    f"(Task Manager on Windows, Activity Monitor on Mac)\n"
    ...
    def save_game(self):
        ...
        if self.save.save_to_game(modded=use_modded):
            ...
            error_detail = getattr(self.save, 'last_error', '') or ''
            messagebox.showerror("Error",
                "Failed to save.\n\n"
                "Make sure PokePath TD is completely closed and try again.\n\n"
