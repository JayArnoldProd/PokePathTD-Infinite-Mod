...
const LEVELDB_PATH = process.platform === 'darwin'
    ? path.join(os.homedir(), `Library/Application Support/${APP_FOLDER}/Local Storage/leveldb`)
    : path.join(os.homedir(), `AppData/Roaming/${APP_FOLDER}/Local Storage/leveldb`);
...
function isGameRunning() {
    try {
        if (process.platform === 'darwin') {
            const output = execSync('pgrep -f "PokéPath TD"', {
                encoding: 'utf8',
                timeout: 5000,
            });
